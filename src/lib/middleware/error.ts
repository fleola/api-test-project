...
function getHttpStatusCoe(error: HttpError, response: Response) {
...
export function initErrorMiddleware(appEnvironment: string): ErrorRequestHandler {
    return function errorMiddleware(error, request, response, next) {
        ...
        const statusCode = getHttpStatusCoe(error, response)
